...
** Highlights: This enhancement does not affect existing commands and commands to be added in future. It is an independent command and it can only be added after the selected flashcards exist in the FlashBook. This gives the user the choice to decide if he/she wants to assign a deadline or a note to the flashcard.
...
* *Minor enhancement*: MnemonicParsing, which registers a keyboard shortcut to activate the buttons, is also added for the two buttons, _T for `topic` and _C for `content`. However, this only works for Windows. (Pull request https://github.com/cs2113-ay1819s2-m11-3/main/pull/86[#86])
